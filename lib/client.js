...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TURN_RAIL_CSS = [
			".dyn-turnrail { z-index: 60; }",
			".dyn-turnrail-track { position: absolute; left: 50%; top: 2px; bottom: 2px; width: calc(var(--dyn-tr-u, 10px) * 0.2); transform: translateX(-50%); border-radius: 2px; background: var(--dsw-alias-border-l2); opacity: 0.45; transition: opacity 120ms ease; }",
			".dyn-turnrail:hover .dyn-turnrail-track { opacity: 0.8; }",
			".dyn-turnrail-item { position: absolute; left: 50%; width: var(--dyn-tr-u, 10px); height: var(--dyn-tr-u, 10px); padding: 0; margin: 0; border: none; border-radius: 999px; background: var(--dsw-alias-label-secondary); opacity: 0.5; cursor: pointer; transform: translate(-50%, -50%); transition: opacity 100ms ease, background 100ms ease; }",
			".dyn-turnrail-item:hover { opacity: 1; background: var(--dsw-alias-brand-primary); }",
			".dyn-turnrail-item.dyn-turnrail-active { opacity: 1; background: var(--dsw-alias-brand-primary); width: calc(var(--dyn-tr-u, 10px) * 1.25); height: calc(var(--dyn-tr-u, 10px) * 1.25); }",
			".dyn-turnrail-tip { position: absolute; right: calc(100% + 12px); top: 50%; transform: translateY(-50%); background: var(--dsw-alias-bg-overlay); border: 1px solid var(--dsw-alias-border-l2); border-radius: 8px; padding: calc(5px + var(--dyn-tr-u, 10px) * 0.12) calc(8px + var(--dyn-tr-u, 10px) * 0.22); font-size: calc(11px + var(--dyn-tr-u, 10px) * 0.25); line-height: calc(16px + var(--dyn-tr-u, 10px) * 0.3); color: var(--dsw-alias-label-primary); width: max-content; max-width: calc(320px + var(--dyn-tr-u, 10px) * 7); overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 8; -webkit-box-orient: vertical; box-shadow: 0 6px 20px rgba(0, 0, 0, 0.16); pointer-events: none; }",
		].join("\n");
		...
		function apply(ctx) {
			...
			function TurnRail() {
				...
				const railH = Math.max(Math.round(unit * 2.8), Math.min(usable, desired));
