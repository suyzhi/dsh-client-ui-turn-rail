			...
			function computeMarkers(snapshot) {
				...
				if (turnOrder !== null && turnOrder.length > 0) {
					...
					for (let i = 0; i < order.length; i++) {
						...
						if (typeof turn !== "number" || firstByTurn.has(turn)) continue;
						firstByTurn.set(turn, node);
					}
					for (let i = 0; i < turnOrder.length; i++) {
						const node = firstByTurn.get(turnOrder[i]);
						if (node === null || node === undefined) continue;
			...
			function TurnRail() {
				...
				for (let i = 0; i < markers.length; i++) {
					...
					if (isHover) {
						...
						else if (pct > 0.85) tipStyle = { bottom: "2px", transform: "none" };
