...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const TURN_RAIL_CSS = [
			...
			".dyn-turnrail-progress { position: absolute; left: 50%; top: 3px; width: calc(var(--dyn-tr-u, 10px) * 0.22); transform: translateX(-50%); border-radius: 999px; background: linear-gradient(180deg, var(--dsw-alias-brand-primary), var(--dsw-alias-brand-primary)); opacity: 0.85; transition: height 220ms ease; }",
		...
		function apply(ctx) {
			...
			function TurnRail() {
				...
				for (let i = 0; i < markers.length; i++) {
					...
					if (pos <= threshold) { activeKey = markers[i].key; activeIndex = i; }
					else break;
				}
				if (activeKey === null) activeKey = markers[0].key;
				const progressPct = (((activeIndex + 0.5) / markers.length) * 100).toFixed(1);
